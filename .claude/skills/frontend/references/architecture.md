...
6. **Nada de un módulo `'use client'` es importable en server-only desde un Server Component** — ni siquiera un `export const` con un literal plano (string, número). El compilador RSC lo convierte en una "client reference" opaca; una función server-only (p. ej. `ReactDOM.preload`, `generateMetadata`) que reciba ese valor no obtiene el literal real. Incidente real (EnduroFun T3.3): un componente `'use client'` exportaba `const HERO_POSTER_SRC = '...'` para uso interno; la página (Server Component) necesitaba ese mismo string para `preload()` y lo importaba directamente — fallaba en silencio, sin el `<link rel="preload">` esperado en el HTML. Fix: la fuente de verdad del literal vive en el Server Component (la página) y baja al client component por prop — el sentido normal de la frontera, nunca al revés.
