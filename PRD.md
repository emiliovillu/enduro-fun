...
- **i18n estático**: rutas localizadas `/en/`, `/es/`, `/de/` generadas vía `generateStaticParams`; sin middleware (incompatible con export estático) — el selector de idioma navega entre rutas ya generadas. Librería concreta (`next-intl` u otra compatible con `output: 'export'`) se decide y verifica en la tarea de F0 que monta i18n `[verificar: compatibilidad exacta con la versión de Next.js del monorepo]`.
