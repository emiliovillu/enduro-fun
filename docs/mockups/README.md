...
| Home — sección Galería | `/[locale]` (entre Packages y Reviews) | `docs/mockups/home-gallery.html` | T1.5 | `docs/mockups/home-gallery.png` |
...
- **Home — Galería (T1.5, 2026-07-19)**: layout acordado vía pregunta directa (contenido: fotos placeholder; posición: entre Packages y Reviews) — título alineado a la izquierda + carrusel horizontal de tarjetas de foto placeholder con scroll-snap y puntos de paginación, mismo ancho de contenedor que Reviews. Único ajuste respecto al mockup estático: el pase automático (autoplay) se pidió después de la aprobación inicial del mockup, junto con un botón de pausa/play explícito — no estaba en el HTML estático (no se puede demostrar JS en un mockup), pero es fiel a "carrusel aesthetic" pedido por el usuario.
