...
- **Subtareas**:
  ...
  - [x] DNS de `endurofun.eu` en Hostinger apuntando a Cloudflare (nameservers `byron.ns.cloudflare.com`/`lilith.ns.cloudflare.com`)
...
- **Actualización 2026-07-23 (mismo día del cierre)**: la zona de Cloudflare pasó a "Activo" y se añadió `endurofun.eu` + `www.endurofun.eu` como Custom Domain del proyecto Pages — resuelto antes de lo previsto, ya NO queda pendiente para F3/T3.4. Confirmado con `curl`: `https://endurofun.eu/`, `/en/`, `/es/`, `/de/` y `https://www.endurofun.eu/` sirven el sitio real (200, `<title>EnduroFun</title>`) con certificado TLS válido (`CN=endurofun.eu`, `SSL certificate verify ok.`).
...
- **Hotfix 2026-07-25** (petición directa del usuario, dato real de negocio — el perfil de Instagram cambió): el enlace y el texto del footer pasan de `@endurofun_oficial` a `@endurofun.eu` (`apps/web/src/components/ui/footer.tsx`). PRD §D7/§14.10 actualizados en consonancia. **Deuda anotada**: `docs/design-system/components/navigation/Footer.jsx` (espejo de solo-lectura de Claude Design) sigue con el handle antiguo — no se edita a mano (regla del proyecto); pendiente de sincronizar el proyecto de Claude Design y regenerar con `DesignSync` en una tarea futura que toque el DS. `pnpm gate` + `pnpm test:e2e` (55/55) verdes.
...
- **Entrega**:
  1. **Diseño primero, en Claude Design** (`8ee30e13-2372-49e4-ba6f-2692bc1a6af5`): nueva card de componente `Lightbox` en `components/media/` (junto a `Icon`/`MapEmbed`) — overlay/scrim sobre el resto de la página, imagen centrada y ampliada respetando su proporción (sin recorte forzado, a diferencia del `object-cover` de las miniaturas del grid), controles de cierre (botón explícito + click en el overlay + `Escape`), fiel a la única identidad visual del DS (sin gradientes/paleta cruda, tokens existentes de color/radios/elevación).
  2. **Subida vía `DesignSync`** (`.jsx` + `.d.ts` + `.prompt.md` + card) y regeneración del espejo local (`docs/design-system/`) — mismo procedimiento que TD.4 con `Input`/`Textarea`.
  3. **Implementación en código** (`apps/web/src/components/ui/lightbox.tsx`, o local a `gallery-grid.tsx` si tras revisar el resto del inventario no hay otro consumidor previsible — decisión a tomar en la tarea, documentar el porqué): accesible (`role="dialog"` + `aria-modal`, foco atrapado dentro mientras está abierto, foco devuelto al elemento que abrió el lightbox al cerrar, operable por teclado — `Escape` cierra, `Tab`/`Shift+Tab` no se escapan del diálogo), sin librería nueva de modal (mismo criterio "sin dependencias nuevas si no hace falta" del resto del proyecto — usar `<dialog>` nativo o un patrón manual con portal). Sección nueva en `/design-system`.
  4. **Integración en Gallery** (`apps/web/src/app/[locale]/gallery/gallery-grid.tsx`): cada foto del grid es clicable/operable por teclado y abre el lightbox con esa foto a tamaño grande (mismo AVIF ya servido, sin generar una segunda variante de mayor resolución salvo que el resultado se vea pixelado en pantallas grandes — a valorar en la tarea).
- **Playwright permanente**: añadir a `apps/web/e2e/gallery.spec.ts` — click en una foto abre el lightbox con overlay visible y la imagen correspondiente; `Escape` cierra y devuelve el foco a la miniatura clicada; click en el overlay (fuera de la imagen) cierra; el foco queda atrapado dentro del diálogo mientras está abierto (`Tab` repetido no sale a elementos de detrás).
- **Verificación**: en navegador, `/en/gallery` (y confirmación cruzada en `/es`/`/de` de que el lightbox funciona igual, sin texto que traducir salvo el `aria-label` del botón de cierre) — clicar varias fotos distintas del grid abre cada una ampliada con el overlay detrás, cerrar con los 3 mecanismos (botón/overlay/Escape) funciona, el foco se comporta según lo descrito arriba; comparación visual contra la card `Lightbox` del espejo de Claude Design; `pnpm gate` verde.

#### TD.12 · Componente FleetCard + sección "Nuestra flota" en About [x] 2026-07-22 — PASS, ver docs/verifications/TD.12/
- **Depende de**: TD.7, T1.2
- **Añadida 2026-07-22** (petición directa del usuario sobre la página About ya cerrada — ver journal): nueva sección "Nuestra flota" justo debajo de la sección "Our story"/guías de `/about`, mismo patrón de scope-change que TD.8-TD.11 (componente nuevo del design system añadido tras el cierre de su fase, no una tarea prevista en el planning original). **Deviación deliberada de `components.md` §2** ("promoción a `ui/` exige ≥2 dominios consumidores"): `FleetCard` tiene hoy un solo consumidor (About) — se promueve igualmente porque el usuario pidió explícitamente que pasara por el flujo de actualización del design system (mismo patrón ya usado en TD.11), no una reinterpretación silenciosa de la regla; anotado aquí y en el journal.
- **Datos reales de la flota** (aportados por el usuario, no inventar cilindradas/modelos): **Husqvarna TE 300** — moto de enduro, 300cc. **Husqvarna Norden 901** — moto de trail/aventura, 901cc.
- **Entrega**:
  1. **Diseño primero, en Claude Design** (`8ee30e13-2372-49e4-ba6f-2692bc1a6af5`): nueva card de componente `FleetCard` en `components/cards/` (junto a `PackageCard`/`ReviewCard`) — mismo lenguaje visual que `PackageCard` (caja de foto placeholder arriba con degradado tokenizado + bloque de texto abajo), props previstas: nombre del modelo, cilindrada, categoría (Enduro / Trail & Adventure), `imageSlot` opcional (mismo patrón que `PackageCard`).
  2. **Subida vía `DesignSync`** (`.jsx` + `.d.ts` + `.prompt.md` + card) y regeneración del espejo local (`docs/design-system/`) — mismo procedimiento que TD.4/TD.11.
  3. **Implementación en código** (`apps/web/src/components/ui/fleet-card.tsx`, props planas, prohibido importar tipos de `@app/core` — mismo criterio que `PackageCard`/`ReviewCard`). Sección nueva en `/design-system`.
  4. **Contrato de dominio**: `FleetBikeSchema` nuevo en `packages/core/src/contracts` (mismo patrón que `PackageSchema`/`ReviewSchema` — nombre del modelo NO traducido por ser nombre propio, cilindrada numérica, categoría enum `enduro | trail-adventure`, descripción `LocalizedTextSchema`) + `apps/web/src/data/fleet.ts` con las 2 motos reales de arriba, validado con `.parse()` a nivel de módulo (control negativo gratis, mismo mecanismo que `packages.ts`).
  5. **Integración en About** (`apps/web/src/app/[locale]/about/page.tsx`): nueva `<section>` justo debajo de la sección "Our story" (guías) y antes de "What makes us different", **mismo layout que la sección de arriba** (`grid lg:grid-cols-2 gap-14 items-center` — pedido explícito del usuario): `SectionHeading` con eyebrow/title nuevos (`about.fleet.*` en `messages.gallery`-sibling namespace) encima del grid, y las 2 `FleetCard` (TE 300 / Norden 901) como las 2 columnas.
- **Playwright permanente**: añadir a `apps/web/e2e/about.spec.ts` — la sección "Nuestra flota" es visible en los 3 idiomas, justo después de la sección de guías/historia y antes de "What makes us different" (orden real del DOM, no solo presencia), muestra las 2 motos con su nombre/cilindrada/categoría correctos.
- **Verificación**: en navegador, `/en/about`, `/es/about`, `/de/about` muestran la sección "Nuestra flota" en la posición correcta, con el mismo layout de 2 columnas que la sección de guías justo arriba, las 2 `FleetCard` con los datos reales (Husqvarna TE 300 300cc enduro, Husqvarna Norden 901 901cc trail/aventura) traducidos en los 3 idiomas salvo los nombres propios; comparación visual contra la card `FleetCard` del espejo de Claude Design; `pnpm gate` verde.
- **Cambio de alcance menor 2026-07-23** (petición directa del usuario, fix directo sin ciclo implementer/verifier — decisión explícita del usuario al preguntarle): se añade una 3ª moto a la flota, **BMW 1300 GS** (trail/aventura, 1300cc — dato real, no inventado), a `apps/web/src/data/fleet.ts`. Reutiliza el `FleetBikeSchema`/`FleetCard` existentes sin cambios de contrato. El grid de la sección pasa de `lg:grid-cols-2` a `sm:grid-cols-2 lg:grid-cols-3` (2 columnas dejaba la 3ª card sola en su fila). `about.spec.ts` actualizado a 3 motos (la etiqueta de categoría "Trail & Adventure"/equivalentes ahora se verifica con `toHaveCount(2)` en vez de un único match). `pnpm gate` verde tras el cambio.
- **Cambio de alcance menor 2026-07-24** (petición directa del usuario, foto real aportada por el usuario — fix directo sin ciclo implementer/verifier, mismo patrón que T1.2/norden-901/te-300): el placeholder tokenizado de la card de la BMW 1300 GS se sustituye por una foto real (`apps/web/public/fleet/bmw-1300-gs.avif`, ~89KB, mismo pipeline `sharp`/AVIF que las otras 2 motos). Sin cambio de contrato ni de layout — solo entrada nueva en `FLEET_IMAGES` (`apps/web/src/data/fleet.ts`). Comprobado visualmente en `/en/about`, `/es/about`, `/de/about`. `pnpm gate` verde tras el cambio.

#### TD.13 · Navegación anterior/siguiente en el Lightbox de Gallery [x] 2026-07-23 — PASS, ver docs/verifications/TD.13/
- **Depende de**: TD.11
- **Añadida 2026-07-23** (petición directa del usuario sobre la Gallery ya cerrada — ver journal): en la vista ampliada de una foto (`Lightbox`, TD.11), añadir flechas de navegación (anterior/siguiente) para poder recorrer las fotos del grid sin cerrar el visor y volver a clicar. Mismo patrón de scope-change que TD.8-TD.12: modifica un componente ya cerrado del design system, así que pasa por el flujo formal de actualización (diseño primero en Claude Design, subida vía `DesignSync`, implementación, cierre con implementer+verifier) — no se implementa directamente en conversación.
- **Estado del código relevante hoy** (para quien la implemente, evita releer todo `lightbox.tsx`/`gallery-grid.tsx` desde cero): `gallery-grid.tsx` ya guarda `openIndex: number | null` (el índice de la foto ampliada actualmente) y se lo pasa a `Lightbox` como `src`/`alt` ya resueltos — añadir prev/next es sobre todo cablear `openIndex - 1` / `openIndex + 1` (con límites: sin wrap-around salvo que el usuario pida circular al implementarla, a decidir en la tarea) y pasarlo a la primitiva. `Lightbox` (`apps/web/src/components/ui/lightbox.tsx`) es props planas sin conocer `@app/core` (components.md §2) — las nuevas props (`onPrev`/`onNext` o equivalente, más sus `aria-label`) deben mantener ese contrato, igual que `closeLabel` ya lo hace hoy.
- **Entrega**:
  1. **Diseño primero, en Claude Design** (`8ee30e13-2372-49e4-ba6f-2692bc1a6af5`): actualizar la card `Lightbox` en `components/media/` con las 2 flechas (izquierda/derecha), mismo lenguaje visual que el botón de cerrar ya existente (icono sobre fondo tokenizado, sin gradientes/paleta cruda). Posición fija a los lados de la imagen, visibles sobre el overlay/scrim.
  2. **Subida vía `DesignSync`** (regenerar `.jsx`/`.d.ts`/`.prompt.md`/card) y regeneración del espejo local (`docs/design-system/`) — mismo procedimiento que TD.11/TD.12.
  3. **Implementación en código** (`apps/web/src/components/ui/lightbox.tsx`): 2 botones de navegación (icono flecha izquierda/derecha, reusar el registro de `Icon` existente si ya hay un glifo adecuado o añadir uno nuevo), ocultos/deshabilitados en el extremo correspondiente si NO es circular (primera foto sin botón "anterior", última sin "siguiente" — salvo que se decida navegación circular en la tarea, documentar la decisión). Operable por teclado: `ArrowLeft`/`ArrowRight` navegan igual que los botones, sin salir del foco atrapado del diálogo (mismo mecanismo de `@base-ui/react/dialog` ya usado en TD.11). Nuevas claves de mensaje (`gallery.lightboxPrevLabel`/`gallery.lightboxNextLabel` o nombres equivalentes, mismo patrón que `gallery.lightboxCloseLabel` de TD.11) en el esquema Zod de `packages/core` + los 3 idiomas.
  4. **Integración en Gallery** (`gallery-grid.tsx`): cablear `openIndex` a los nuevos props de navegación del Lightbox.
- **Playwright permanente**: extender `apps/web/e2e/gallery.spec.ts` (o el fichero donde viven hoy los tests de TD.11) — clicar "siguiente" avanza a la foto correcta (`src`/`alt` cambian), clicar "anterior" retrocede, `ArrowRight`/`ArrowLeft` del teclado hacen lo mismo sin perder el foco atrapado, el botón/límite correspondiente no navega más allá de la primera/última foto (o si se decide circular, vuelve al otro extremo — ajustar el test a la decisión tomada).
- **Verificación**: en navegador, `/en/gallery` (y confirmación cruzada en `/es`/`/de` de que las flechas funcionan igual, sin más texto que traducir que sus `aria-label`) — abrir una foto del grid, navegar con las flechas (ratón y teclado) por varias fotos consecutivas en ambas direcciones, comprobar el comportamiento en los extremos (primera/última foto) según la decisión tomada en la tarea; comparación visual contra la card `Lightbox` actualizada del espejo de Claude Design; `pnpm gate` verde.
- **Decisiones tomadas**: navegación **sin wrap-around** (primera foto sin botón "anterior", última sin "siguiente"); límites calculados sobre las 122 fotos reales (`TOTAL_PHOTOS`), no sobre las cargadas por scroll infinito; glifos nuevos `chevron-left`/`chevron-right` añadidos al registro de `Icon`.
- **Deuda anotada**: (1) patrón "botón icono circular" ya son 5 instancias en el proyecto (header, carrusel de Home, cerrar/anterior/siguiente del Lightbox) — candidato a `IconButton` del DS, no aplicado por alcance. (2) naming de iconos no uniforme (`chevron-left`/`map-pin` kebab-case vs `chevronDown` camelCase) — preexistente a esta tarea, no bloqueante. (3) `docs/design-system/_adherence.oxlintrc.json` no quedó sincronizado por `DesignSync` con los componentes/iconos nuevos (`Lightbox` ausente del catálogo desde TD.11; `chevron-left`/`chevron-right` ausentes del enum de `Icon.name`) — gap de la propia tool, no del código de esta tarea. (4) sin test e2e del caso "clic en Next repetido hasta que el botón desaparece" (cubierto por teclado, no por ratón) — cobertura menor, no bloqueante.
...
- **Cambio de alcance menor 2026-07-25** (petición directa del usuario, vídeo real aportado por el usuario — fix directo sin ciclo implementer/verifier completo, pero con pases `code-review`/`simplify`/`ds-reviewer` por introducir lógica nueva, no solo un asset estático): el placeholder de foto/vídeo del hero se sustituye por un vídeo real en autoplay silenciado/loop. Origen: clip vertical (9:16, 1080×1920, HEVC, ~2:30, 135MB). Recortado a los primeros 42s (evita un tramo de interior/taller fuera de marca en el resto del clip) y reencuadrado a 16:9 (`crop=1080:608:0:750`, offset elegido comparando fotogramas de muestra de todo el vídeo — evita que el crop se quede solo con cielo) vía `ffmpeg`; recodificado a H.264 854×480 sin audio, crf 30, `+faststart` → `apps/web/public/hero/home-hero.mp4` (~6.8MB, -95% del original, mismo espíritu de compresión agresiva que el pipeline AVIF de fotos). Poster `apps/web/public/hero/home-hero-poster.avif` generado con `sharp` desde un fotograma del propio vídeo ya recodificado. Nuevo componente `apps/web/src/app/[locale]/home-hero-video.tsx` (client, mismo patrón que `HomePhotoCarousel`): `.play()` disparado tras montar solo si `prefers-reduced-motion` no está activo (autoplay estático no sirve porque el SSR no conoce la preferencia del cliente). `apps/web/e2e/home.spec.ts` con test nuevo permanente (atributos `muted`/`loop`/`poster` + `currentTime` avanzando de verdad, no solo `paused === false`). **`code-review`**: sin bloqueantes; 1 hallazgo de observabilidad (el `.catch()` del `.play()` tragaba TODO rechazo, no solo el `NotAllowedError` esperado de la política de autoplay) — corregido en el momento, no diferido (regla dura del proyecto). **`simplify`**: aplicado — `preload="auto"` → `"metadata"` (el `poster` ya cubre el primer pintado, no competir con el LCP) y el `useState` de `reducedMotion` eliminado (se lee una vez dentro del propio `useEffect`, no necesita vivir en el árbol de estado). **Deuda anotada**: el patrón `matchMedia('(prefers-reduced-motion: reduce)')` ya se repite en `home-photo-carousel.tsx` y `gallery-grid.tsx` (más este componente) — candidato a hook compartido `useReducedMotion()`, no extraído aquí por tocar 2 componentes ya cerrados y verificados, fuera de alcance de este cambio. **`ds-reviewer`**: veredicto LIMPIO (no existe primitiva de vídeo en el DS, el `<video>` full-bleed es HTML crudo legítimo por ausencia de equivalente). Verificado visualmente en `/en/`, `/es/`, `/de/` con capturas Playwright ad hoc (vídeo reproduciendo tras cargar, antes y después de los fixes). `pnpm gate` + `pnpm test:e2e` (54/54) verdes.
- **Cambio de alcance menor 2026-07-25 (bis)** (petición directa del usuario: "recorta el vídeo para que empiece justo después de que desaparezca el logo de EnduroFun"): el clip fuente abre con una animación de logo grande y centrado que se disuelve del todo hacia `t≈4.8s` (verificado fotograma a fotograma, en pasos de 0.2s cerca de la transición) — a partir de ahí solo queda un watermark pequeño de esquina que la propia ventana de crop (`crop=1080:608:0:750`) ya deja fuera de encuadre, así que no hace falta tratarlo aparte. Regenerado `home-hero.mp4`/`home-hero-poster.avif` con el mismo pipeline pero `-ss 4.8` (en vez de `0`) sobre el clip original, mismos 42s de duración, mismo crop/bitrate (~6.9MB). Verificado visualmente en desktop (1440px) y mobile (390px, iPhone) que el primer fotograma ya no muestra nada del logo. `pnpm gate` + `pnpm test:e2e` (55/55) verdes.
- **Cambio de alcance menor 2026-07-25 (ter)** (petición directa del usuario: "haz lo mismo en la sección de los Paquetes de la Home"): mismo `packageImageSlot()` de T2.1 (`@/data/packages`) cableado también en la preview de 3 `PackageCard` de Home — una sola línea (`imageSlot={packageImageSlot(pkg.id)}`), sin datos nuevos, mismas 3 fotos que ya usa `/packages`. Verificado visualmente en `/en/`. `pnpm gate` + `pnpm test:e2e` (55/55) verdes.
...
- **Entrega**: `/[en|es|de]/contact` con formulario (`Input` nombre/email + `Textarea` mensaje de TD.4) que hace POST directo al endpoint de Formspree; texto invitando a pedir presupuesto personalizado; `MapEmbed` con el iframe **real** de Google Maps (ubicación de Álora, Málaga) — cierra el `[verificar]` de coste/condiciones de PRD §9.1. Cierra también el `[verificar]` de límite del plan gratuito de Formspree (PRD §9.2). **Decisión 2026-07-20**: el mapa real usa el endpoint público sin API key (`maps.google.com/maps?...&output=embed`) en vez de la Maps Embed API oficial — decisión explícita del usuario tras comparar ambas opciones (cero prerequisitos externos vs. respaldo/SLA oficial de Google), documentada en PRD §9.1 y en `map-embed.tsx`. Ya no hay ningún prerequisito ⚠ externo pendiente para esta tarea.
- **Subtareas**:
  ...
  - [x] Mapa real de Álora, Málaga — resuelto 2026-07-20 sin API key (ver decisión arriba), `MapEmbed` con prop `interactive`, también activado en Home (T1.1) para consistencia visual
  - [x] Formulario con validación mínima (campos requeridos) usando `Input`/`Textarea` del DS
  - [x] Iframe de Google Maps embebido — implementado sin key, ver arriba
- **Mockup**: `docs/mockups/contact.html` (creado y aprobado por el usuario 2026-07-20; layout final vuelve al grid de 2 columnas del mockup tras una iteración intermedia que apilaba formulario/mapa en secciones separadas — el usuario pidió revertirlo).
- **Playwright permanente**: `apps/web/e2e/contact.spec.ts` — el formulario se rellena y envía contra un **fixture/mock del endpoint de Formspree** (nunca el endpoint real en CI, regla 10); se comprueba el estado de éxito mostrado al usuario; el mapa embebido (iframe real) está presente en el DOM (sin verificar contenido del iframe de terceros).
- **Verificación**: en navegador, rellenar y enviar el formulario real una vez contra el endpoint real de Formspree → comprobar que el email llega a la bandeja configurada (evidencia: captura del email recibido); el mapa de Google muestra Álora, Málaga, visible e interactivo (zoom/paneo funcionan). Este envío real es manual/one-shot (no se repite en cada gate — el gate usa el fixture de Playwright).
  - **Estado 2026-07-20** (ver `docs/verifications/T1.3/report.md`): PASS parcial en todo lo automatable + el envío real one-shot contra Formspree (POST 200 confirmado por captura de red) + mapa real implementado. Queda **1 pendiente** que solo puede resolver el usuario antes de poder marcar esta tarea `[x]`: confirmar que el email de prueba llegó a `info@endurofun.eu` (el usuario ha indicado que esa bandeja todavía no existe — está pendiente de que el cliente confirme la dirección de contacto definitiva; si cambia, es un ajuste de un solo valor en el dashboard de Formspree, sin tocar código). La tarea NO se cierra hasta que se confirme el envío real a la bandeja definitiva.
...
- **Cambio de alcance menor 2026-07-24** (petición directa del usuario — "añade 5 fotos, las que tú quieras, de las que están en la galería" — fix directo sin ciclo implementer/verifier, mismo patrón que TD.12/T1.2): las 5 tarjetas placeholder se sustituyen por 5 fotos reales elegidas a mano entre las 122 de Gallery (`gallery-001`, `gallery-019`, `gallery-043`, `gallery-055`, `gallery-121` — variadas a propósito: paisaje icónico del Caminito del Rey, acción en pareja junto a cascada, foto de grupo, dos riders en valle y rider en solitario), referenciando directamente los ficheros ya existentes de `apps/web/public/gallery/` (sin duplicar assets). Nueva clave `home.gallery.photoAltTemplate` en los 3 idiomas + `MessagesSchema` (`packages/core/src/contracts/messages.ts`) actualizado con control negativo. Verificado visualmente en `/en/`, `/es/`, `/de/`; `pnpm gate` + `pnpm test:e2e` (home.spec.ts, 53/53) verdes.
- **Cambio de alcance menor 2026-07-25** (petición directa del usuario — bug real reportado: "solo se ven las 5 mismas fotos, pero no se mueven"): con exactamente 5 tarjetas y la clase `grow`, en viewports anchos (≥~1700px) las 5 se repartían todo el ancho del track sin dejar overflow — el autoplay cambiaba `activeIndex`/los dots pero `scrollTo()` no tenía nada que desplazar, sin movimiento visible. Fix: 5 fotos más (10 en total, `GALLERY_PHOTO_INDEXES` en `home-photo-carousel.tsx`) elegidas a mano entre las 122 de Gallery (`gallery-017`, `gallery-029`, `gallery-045`, `gallery-061`, `gallery-113` — panorámica sin gente, acción en barranco, 2 riders al atardecer, grupo con vistas nubladas, rider entre vegetación dorada), mismo criterio de variedad que la 1ª tanda. Con 10 × 340px = 3400px el `grow` deja de tener sobrante que repartir, el track queda con overflow real y el desplazamiento vuelve a ser visible — el mecanismo de scroll ya era correcto, el fix es de CONTENIDO, no de lógica. `apps/web/e2e/home.spec.ts` actualizado (10 dots en vez de 5) + **test nuevo permanente de control negativo** que comprueba `scrollWidth > clientWidth` y que `track.scrollLeft` avanza de verdad tras un tick de autoplay — verificado manualmente que se pone en ROJO reintroduciendo el bug (volviendo a 5 fotos) antes de fijar el fix. Verificado visualmente con capturas del `scrollLeft` real avanzando (0→340→680px) en `/en/`. `pnpm gate` + `pnpm test:e2e` (55/55) verdes.
...
- **Cambio de alcance menor 2026-07-25** (petición directa del usuario, fotos reales elegidas por criterio explícito del usuario — fix directo sin ciclo implementer/verifier): las 3 `PackageCard` de esta página ganan foto real vía `imageSlot` (mismo escape hatch ya usado por `FleetCard`), reusando ficheros YA existentes de `apps/web/public/gallery/` sin duplicar assets. `own-bike` → `gallery-097.avif` (almacén de la empresa, fila de motos guardadas — pedido explícito para el paquete de "trae tu propia moto"); `getaway` → `gallery-049.avif` y `full-adventure` → `gallery-033.avif` (pilotos en ruta, pedido explícito para los otros 2 paquetes). Nuevo `packageImageSlot()` en `apps/web/src/data/packages.ts`, mismo patrón que `fleetImageSlot()`. Verificado visualmente en `/en/packages`, `/es/packages`, `/de/packages`. `pnpm gate` + `pnpm test:e2e` (55/55) verdes.
